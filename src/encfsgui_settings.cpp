...
public:
	//ctor
	frmSettingsDialog(wxWindow *parent, wxString& title, const wxPoint& pos, const wxSize& size, long style);
	void Create();
...
frmSettingsDialog::frmSettingsDialog(wxWindow *parent, wxString& title, const wxPoint& pos, const wxSize& size, long style) : wxDialog(parent, wxID_ANY, title, pos, size, style)
{
...
{	
	...
	frmSettingsDialog* dlg = new frmSettingsDialog(parent, strTitle, wxDefaultPosition, dlgSettingsSize, style);
    dlg->Create();
    dlg->ShowModal();
}
